...
static int
cmd_writenand_run (urj_chain_t *chain, char *params[])
{
    long unsigned adr;
    long unsigned len;
    long unsigned isecc;
    ...
    if (urj_cmd_params (params) != 5)
    {
        urj_error_set (URJ_ERROR_SYNTAX,
                       ...
                       params[0], 5, urj_cmd_params (params));
    ...
    if (urj_cmd_get_number (params[1], &adr) != URJ_STATUS_OK
    	...
    	|| urj_cmd_get_number (params[4], &isecc) != URJ_STATUS_OK)
        return URJ_STATUS_FAIL;
    ...
    r = urj_bus_writenand (urj_bus, f, adr, len, isecc);
    fclose (f);
...
static void
cmd_writenand_complete (urj_chain_t *chain, char ***matches, size_t *match_cnt,
...
cmd_writenand_help (void)
{
    urj_log (URJ_LOG_LEVEL_NORMAL,
             _("Usage: %s ADDR LEN FILENAME\n"
               ...
               "ISECC	0 if input doesn't contain ecc metadata, 1 otherwise\n"
               "\n"
...
const urj_cmd_t urj_cmd_writenand = {
    "writenand",
    N_("write content of file to the memory"),
    cmd_writenand_help,
    cmd_writenand_run,
    cmd_writenand_complete,
};
