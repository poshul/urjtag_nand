    ...
    if (params != NULL)
        ...
        for (i = 0; params[i] != NULL; i++)
        {
            ...
            case URJ_CABLE_PARAM_KEY_TRST:
                cable_params->jtag_gpios[GPIO_TRST] = params[i]->value.lu;
                break;
            case URJ_CABLE_PARAM_KEY_RESET:
                cable_params->jtag_gpios[GPIO_RESET] = params[i]->value.lu;
                break;
            default:
                break;
            }
...
gpio_set_signal (urj_cable_t *cable, int mask, int val)
{
    ...
    if (mask != 0)
    {
        ...
        if (mask & URJ_POD_CS_TRST)
        	gpio_set_value (p->fd_gpios[GPIO_TRST], val & URJ_POD_CS_TRST);
        if (mask & URJ_POD_CS_RESET)
        	gpio_set_value (p->fd_gpios[GPIO_RESET], val & URJ_POD_CS_RESET);
    }
